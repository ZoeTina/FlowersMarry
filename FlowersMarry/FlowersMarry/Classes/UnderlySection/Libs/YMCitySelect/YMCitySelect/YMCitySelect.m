...
-(void)ym_setSearchBar{
    _ym_searchBar = [[YMSearchBar alloc] initWithFrame:CGRectMake(0, 44, self.view.ym_width, 70)];
    _ym_searchBar.backgroundColor = kWhiteColor;
...
-(void)ym_setNavView{
    _ym_navView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kNavBarHeight)];
    _ym_navView.backgroundColor = kWhiteColor;
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(0, 63.5, _ym_navView.ym_width, 0.5)];
    lineView.backgroundColor = kColorWithRGB(221, 221, 221);
...
-(void)ym_setTableView{
    ...
    if([_ym_tableView respondsToSelector:@selector(setSectionIndexColor:)]) {
        //修改索引颜色
        _ym_tableView.sectionIndexBackgroundColor = [UIColor clearColor];//修改右边索引的背景色
        _ym_tableView.sectionIndexColor = [UIColor lz_colorWithHexString:@"#CCCCCC"];//修改右边索引字体的颜色
        _ym_tableView.sectionIndexTrackingBackgroundColor = [UIColor clearColor];//修改右边索引点击时候的背景色
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    ym_view.backgroundColor = kWhiteColor;
    UILabel *ym_label = [[UILabel alloc] init];
    ym_label.textAlignment = NSTextAlignmentLeft;
    ym_label.textColor = kTextColor153;
    ym_label.font = kFontSizeMedium13;
    YMCityGroupsModel *cityGroupModel = _ym_ctiyGroups[section];
