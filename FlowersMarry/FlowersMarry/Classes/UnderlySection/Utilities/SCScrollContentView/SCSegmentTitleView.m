...
@interface SCSegmentTitleView ()<UIScrollViewDelegate>
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [SCSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [SCSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [SCSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
...
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(SCSegmentTitleViewWillBeginDragging:)]) {
        [self.delegate SCSegmentTitleViewWillBeginDragging:self];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    if (self.delegate&&[self.delegate respondsToSelector:@selector(SCSegmentTitleViewWillEndDragging:)]) {
        [self.delegate SCSegmentTitleViewWillEndDragging:self];
    }
}
