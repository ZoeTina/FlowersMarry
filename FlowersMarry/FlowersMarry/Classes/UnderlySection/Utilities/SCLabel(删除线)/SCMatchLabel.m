...
#import "SCMatchLabel.h"

@interface SCMatchLabel()
...
/** 点击类型 */
@property(nonatomic, assign)SCHandleStyle tapStyle;
...
- (void)setMatchArr:(NSArray<NSDictionary *> *)matchArr{
    _matchArr = matchArr;
    [self prepareText];
}
...
-(void)setup{
    ...
    _textColorNormal = [UIColor colorWithRed:162.0/255 green:162.0/255  blue:162.0/255  alpha:162.0/255];
    _textHightLightBackgroundColor = [UIColor colorWithWhite:0.7 alpha:0.2];
...
-(void)drawRect:(CGRect)rect{
    ...
    if (self.selectedRange.length != 0) {
        ...
        // 2.0.确定颜色
        UIColor *selectedColor = self.isSelected ? self.textHightLightBackgroundColor : [UIColor clearColor];
...
-(void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    switch (self.tapStyle) {
        case SCHandleStyleAgreement:{
            ...
            if (self.tapOperation) {
                ...
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SCHandleStyleAgreement, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SCHandleStyleLink:{
            ...
            if (self.tapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SCHandleStyleLink, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SCHandleStyleTopic:{
            ...
            if (self.tapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SCHandleStyleTopic, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SCHandleStyleUser:{
            ...
            if (self.tapOperation) {
                ...
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SCHandleStyleUser, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
        case SCHandleStyleUserDefine:{
            __weak typeof(self) weakSelf = self;
            if (self.tapOperation) {
                __strong typeof(weakSelf) strongSelf = weakSelf;
                if (!strongSelf) return;
                if (strongSelf.tapOperation) {
                    strongSelf.tapOperation(strongSelf, SCHandleStyleUserDefine, selectedString, strongSelf.selectedRange);
                }
            }
        }
            break;
    ...
    // 4.代理
    if ([self.delegate respondsToSelector:@selector(sc_label:didSelectedString:forStyle:inRange:)]) {
        [self.delegate sc_label:self didSelectedString:selectedString forStyle:self.tapStyle inRange:self.selectedRange];
    }
...
-(NSRange)getSelectRange:(CGPoint)selectPoint{
    ...
    for (NSValue *value in self.linkRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.tapStyle = SCHandleStyleLink;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.tapStyle = SCHandleStyleUser;
            return range;
        }
    }
    ...
    for (NSValue *value in self.topicRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.tapStyle = SCHandleStyleTopic;
            return range;
        }
    }
    ...
    for (NSValue *value in self.agreementRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.tapStyle = SCHandleStyleAgreement;
            return range;
        }
    }
    ...
    for (NSValue *value in self.userDefineRangesArr) {
        ...
        if (index > range.location && index < range.location + range.length) {
            [self setNeedsDisplay];
            self.tapStyle = SCHandleStyleUserDefine;
            return range;
        }
    }
