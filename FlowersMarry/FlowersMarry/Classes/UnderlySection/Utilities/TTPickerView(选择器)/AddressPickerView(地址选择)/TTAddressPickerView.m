...
#import "TTAddressPickerView.h"
#import "TTPickerViewMacro.h"

@interface TTAddressPickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>
{
...
// 显示类型
@property (nonatomic, assign) TTAddressPickerMode showType;
// 选中的省
@property(nonatomic, strong) TTProvinceModel *selectProvinceModel;
// 选中的市
@property(nonatomic, strong) TTCityModel *selectCityModel;
// 选中的区
@property(nonatomic, strong) TTAreaModel *selectAreaModel;
...
// 选中后的回调
@property (nonatomic, copy) TTAddressResultBlock resultBlock;
// 取消选择的回调
@property (nonatomic, copy) TTAddressCancelBlock cancelBlock;

@end

@implementation TTAddressPickerView
...
+ (void)showAddressPickerWithDefaultSelected:(NSArray *)defaultSelectedArr
    ...
    [self showAddressPickerWithShowType:TTAddressPickerModeArea dataSource:nil defaultSelected:defaultSelectedArr isAutoSelect:NO themeColor:nil resultBlock:resultBlock cancelBlock:nil];
}
...
+ (void)showAddressPickerWithDefaultSelected:(NSArray *)defaultSelectedArr
                                isAutoSelect:(BOOL)isAutoSelect
                                 ...
                                 resultBlock:(TTAddressResultBlock)resultBlock {
    [self showAddressPickerWithShowType:TTAddressPickerModeArea dataSource:nil defaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:nil];
}
...
+ (void)showAddressPickerWithShowType:(TTAddressPickerMode)showType
                      ...
                      defaultSelected:(NSArray *)defaultSelectedArr
                         isAutoSelect:(BOOL)isAutoSelect
                          ...
                          resultBlock:(TTAddressResultBlock)resultBlock
                          cancelBlock:(TTAddressCancelBlock)cancelBlock {
    TTAddressPickerView *addressPickerView = [[TTAddressPickerView alloc] initWithShowType:showType dataSource:dataSource defaultSelected:defaultSelectedArr isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (instancetype)initWithShowType:(TTAddressPickerMode)showType
                      dataSource:(NSArray *)dataSource
                 defaultSelected:(NSArray *)defaultSelectedArr
                    isAutoSelect:(BOOL)isAutoSelect
                      themeColor:(UIColor *)themeColor
...
- (void)loadData {
    ...
    if (!self.dataSource || self.dataSource.count == 0) {
        /*
            ...
            然后再去找下面名为 TTPickerView 的 bundle 对象。
         */
        NSBundle *bundle = [NSBundle bundleForClass:[self class]];
        NSURL *url = [bundle URLForResource:@"TTPickerView" withExtension:@"bundle"];
        NSBundle *plistBundle = [NSBundle bundleWithURL:url];
        
        NSString *filePath = [plistBundle pathForResource:@"TTCity" ofType:@"plist"];
...
- (void)parseDataSource {
    ...
    for (NSDictionary *proviceDic in self.dataSource) {
        TTProvinceModel *proviceModel = [[TTProvinceModel alloc]init];
        ...
        for (NSDictionary *cityDic in citylist) {
            TTCityModel *cityModel = [[TTCityModel alloc]init];
            ...
            for (NSDictionary *areaDic in arealist) {
                TTAreaModel *areaModel = [[TTAreaModel alloc]init];
...
- (void)setupDefaultValue {
    ...
    if (self.showType == TTAddressPickerModeCity || self.showType == TTAddressPickerModeArea) {
    ...
    if (self.showType == TTAddressPickerModeArea) {
...
- (void)scrollToRow:(NSInteger)provinceIndex secondRow:(NSInteger)cityIndex thirdRow:(NSInteger)areaIndex {
    if (self.showType == TTAddressPickerModeProvince) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
    } else if (self.showType == TTAddressPickerModeCity) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:cityIndex inComponent:1 animated:YES];
    } else if (self.showType == TTAddressPickerModeArea) {
        [self.pickerView selectRow:provinceIndex inComponent:0 animated:YES];
        [self.pickerView selectRow:cityIndex inComponent:1 animated:YES];
...
- (NSArray *)getCityModelArray:(NSInteger)provinceIndex {
    TTProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    // 返回城市模型数组
...
- (NSArray *)getAreaModelArray:(NSInteger)provinceIndex cityIndex:(NSInteger)cityIndex {
    TTProvinceModel *provinceModel = self.provinceModelArr[provinceIndex];
    TTCityModel *cityModel = provinceModel.citylist[cityIndex];
    // 返回地区模型数组
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.showType) {
        case TTAddressPickerModeProvince:
            return 1;
            break;
        case TTAddressPickerModeCity:
            return 2;
            break;
        case TTAddressPickerModeArea:
            return 3;
            break;
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (component == 0) {
        TTProvinceModel *model = self.provinceModelArr[row];
        label.text = model.name;
    }else if (component == 1){
        TTCityModel *model = self.cityModelArr[row];
        label.text = model.name;
    }else if (component == 2){
        TTAreaModel *model = self.areaModelArr[row];
        label.text = model.name;
    }
    return bgView;
}
...
- (TTProvinceModel *)selectProvinceModel {
    if (!_selectProvinceModel) {
        _selectProvinceModel = [[TTProvinceModel alloc]init];
    }
    return _selectProvinceModel;
}

- (TTCityModel *)selectCityModel {
    if (!_selectCityModel) {
        _selectCityModel = [[TTCityModel alloc]init];
...
- (TTAreaModel *)selectAreaModel {
    if (!_selectAreaModel) {
        _selectAreaModel = [[TTAreaModel alloc]init];
