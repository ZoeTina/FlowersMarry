...
@implementation TTBasePickerView
...
- (UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, kScreenHeight - kTopViewHeight - kPickerHeight - TT_BOTTOM_MARGIN, kScreenWidth, kTopViewHeight + kPickerHeight + TT_BOTTOM_MARGIN)];
...
- (UIView *)topView {
    if (!_topView) {
        ...
        _topView.backgroundColor = kTTToolBarColor;
        // 设置子视图的大小随着父视图变化
...
- (UIButton *)leftBtn {
    if (!_leftBtn) {
        ...
        _leftBtn.backgroundColor = kTTToolBarColor;
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        _rightBtn.backgroundColor = kTTToolBarColor;
...
- (UIView *)lineView {
    if (!_lineView) {
        ...
        _lineView.backgroundColor = UIColorHexString(0xf1f1f1);
...
- (void)setupThemeColor:(UIColor *)themeColor {
    ...
    [self.leftBtn setTitleColor:kTextColor51 forState:UIControlStateNormal];
    
    self.rightBtn.backgroundColor = themeColor;
    self.rightBtn.layer.cornerRadius = 6.0f;
    self.rightBtn.layer.masksToBounds = YES;
    [self.rightBtn setTitleColor:kTextColor51 forState:UIControlStateNormal];
    
    self.titleLabel.textColor = [themeColor colorWithAlphaComponent:0.8f];
}

- (void)dealloc {
    TTLog(@"%@ dealloc", NSStringFromClass([self class]));
}
