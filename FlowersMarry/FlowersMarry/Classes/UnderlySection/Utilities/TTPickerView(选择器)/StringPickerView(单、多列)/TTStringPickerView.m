...
#import "TTStringPickerView.h"
#import "TTPickerViewMacro.h"

typedef NS_ENUM(NSInteger, TTStringPickerMode) {
    TTStringPickerComponentSingle,  // 单列
    TTStringPickerComponentMore     // 多列
};

@interface TTStringPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
{
...
@property (nonatomic, assign) TTStringPickerMode type;
...
@property (nonatomic, copy) TTStringResultBlock resultBlock;
@property (nonatomic, copy) TTStringCancelBlock cancelBlock;

@end

@implementation TTStringPickerView

#pragma mark - 1.显示自定义字符串选择器
+ (void)showStringPickerWithTitle:(NSString *)title
                       dataSource:(id)dataSource
                  defaultSelValue:(id)defaultSelValue
                      resultBlock:(TTStringResultBlock)resultBlock {
...
+ (void)showStringPickerWithTitle:(NSString *)title
                  ...
                  defaultSelValue:(id)defaultSelValue
                     isAutoSelect:(BOOL)isAutoSelect
                       themeColor:(UIColor *)themeColor
                      resultBlock:(TTStringResultBlock)resultBlock
                      cancelBlock:(TTStringCancelBlock)cancelBlock {
    TTStringPickerView *strPickerView = [[TTStringPickerView alloc]initWithTitle:title dataSource:dataSource defaultSelValue:defaultSelValue isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (void)configDataSource:(id)dataSource defaultSelValue:(id)defaultSelValue {
    ...
    if ([[self.dataSourceArr firstObject] isKindOfClass:[NSString class]]) {
        self.type = TTStringPickerComponentSingle;
    } else if ([[self.dataSourceArr firstObject] isKindOfClass:[NSArray class]]) {
        self.type = TTStringPickerComponentMore;
    }
    // 4. 给选择器设置默认值
    if (self.type == TTStringPickerComponentSingle) {
    ...
    } else if (self.type == TTStringPickerComponentMore) {
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView {
    switch (self.type) {
        case TTStringPickerComponentSingle:
            return 1;
            break;
        case TTStringPickerComponentMore:
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (self.type == TTStringPickerComponentSingle) {
        ...
        if (count==2) {
            CGFloat width = self.alertView.frame.size.width / 3;
            label.frame = CGRectMake(0, width/2, width, 35.0f * kScaleFit);
        }else{
