...
- (void)setScaleTagInSort:(CGFloat)scaleTagInSort
{
    if (_scaleTagInSort < 1) {
        @throw [NSException exceptionWithName:@"MVError" reason:@"(scaleTagInSort)缩放比例必须大于1" userInfo:nil];
    }
...
- (void)addTags:(NSArray *)tagStrs
{
    if (self.frame.size.width == 0) {
        @throw [NSException exceptionWithName:@"MVError" reason:@"先设置标签列表的frame" userInfo:nil];
    }
...
{
    Class tagClass = _tagClass?_tagClass : [MVTagButton class];
    
    // 创建标签按钮
    MVTagButton *tagButton = [tagClass buttonWithType:UIButtonTypeCustom];
    if (_tagClass == nil) {
...
{
    // 获取对应的标题按钮
    MVTagButton *button = self.tags[tagStr];
...
- (void)updateTagButtonFrame:(NSInteger)i extreMargin:(BOOL)extreMargin
{
    ...
    // 获取当前按钮
    MVTagButton *tagButton = self.tagButtons[i];
    // 判断是否设置标签的尺寸
