...
#import "SCPlaybackControls.h"

/** 播放器的播放状态 */
typedef NS_ENUM(NSInteger, SCVideoPlayerState) {
    SCVideoPlayerStateFailed,     // 播放失败
    SCVideoPlayerStateBuffering,  // 缓冲中
    SCVideoPlayerStatePlaying,    // 播放中
    SCVideoPlayerStatePause,      // 暂停播放
};

@interface SCVideoPlayer()<SCPlaybackControlsDelegate>
...
/** 播放器配置信息 */
@property (nonatomic, strong) SCPlayerConfiguration *playerConfiguration;
/** 视频播放控制面板 */
@property (nonatomic, strong) SCPlaybackControls *playbackControls;
/** 非全屏状态下播放器 superview */
...
/** 播放器的播放状态 */
@property (nonatomic, assign) SCVideoPlayerState playerState;
/** 是否结束播放 */
@property (nonatomic, assign) BOOL playDidEnd;

@end

@implementation SCVideoPlayer
...
- (void)startPlayVideo{
    if (self.playDidEnd && self.playbackControls.videoSlider.value == 1.0) {
        ...
        if (self.playerState == SCVideoPlayerStatePause) {
            self.playerState = SCVideoPlayerStatePlaying;
        }
    }
}
...
- (void)pausePlayVideo{
    ...
    if (self.playerState == SCVideoPlayerStatePlaying) {
        self.playerState = SCVideoPlayerStatePause;
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"loadedTimeRanges"]){
        ...
        if (self.playerItem.playbackBufferEmpty) {
            self.playerState = SCVideoPlayerStateBuffering;
    ...
    {
        // 当视频缓冲好时
        if (self.playerItem.playbackLikelyToKeepUp && self.playerState == SCVideoPlayerStateBuffering){
            self.playerState = SCVideoPlayerStatePlaying;
        }
    }
    else if ([keyPath isEqualToString:@"status"])
    {
        if (self.player.currentItem.status == AVPlayerStatusReadyToPlay) {
            ...
            self.playerState = SCVideoPlayerStateFailed;
        }
    }
}
 ...
 */
- (void)_setVideoGravity:(SCVideoGravity)videoGravity
{
 ...
 */
- (void)setPlayerState:(SCVideoPlayerState)playerState
{
    _playerState = playerState;
    switch (_playerState) {
        case SCVideoPlayerStateBuffering:
        {
            [_playbackControls _activityIndicatorViewShow:YES];
        }
            break;
        case SCVideoPlayerStatePlaying:
        {
            [_playbackControls _activityIndicatorViewShow:NO];
        }
            break;
        case SCVideoPlayerStateFailed:
        {
            [_playbackControls _activityIndicatorViewShow:NO];
...
/** 播放器控制面板 */
- (SCPlaybackControls *)playbackControls
{
    if (!_playbackControls) {
        _playbackControls = [[SCPlaybackControls alloc]init];
...
- (void)doubleTapGesture
{
    if (_playerConfiguration.supportedDoubleTap) {
        ...
        else if (self.playerState == SCVideoPlayerStatePause)
        {
            [self startPlayVideo];
        }
    }
...
#pragma mark 滑杆拖动代理
/** 开始拖动 */
-(void)videoSliderTouchBegan:(SCVideoSlider *)slider{
    [self pausePlayVideo];
    [_playbackControls _playerCancelAutoHidePlaybackControls];
}
/** 结束拖动 */
-(void)videoSliderTouchEnded:(SCVideoSlider *)slider{
...
/** 拖拽中 */
-(void)videoSliderValueChanged:(SCVideoSlider *)slider{
